...
import { VRAT_CALENDAR_2026, ObservanceItem } from '@/lib/vrat-calendar-data';
...
export async function getTodayLivePanchangServer() {
...
export async function getVratCalendarServer(year = 2026): Promise<ObservanceItem[]> {
  try {
    const generated = generateVratCalendar(year);
    if (generated && generated.length > 0) return generated;
  } catch (e) {}

  return VRAT_CALENDAR_2026;
}
